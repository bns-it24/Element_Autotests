...
def send_keys_to_element(driver, by, value, keys):
    ...
    element = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((by, value)))  # Проверка видимости элемента
...
def add_res_string(driver, arrival_date, room_count, adults, children, guest_category, rate, room_type, payment_info, guest_name, contact_name, booking_source):
...
def edit_and_delete_reservation(driver):
...
def test_save_clear_reservation(driver):
    add_res_string( #Простое бронирование в одну строку с оплатой картой
        driver,
    ...
    add_res_string( # Простое бронирование в одну строку с оплатой наличными
        driver,
    ...
    edit_and_delete_reservation(driver)


def test_add_three_res_strings(driver):
    ...
    # Отмена третьеё строки бронирования
